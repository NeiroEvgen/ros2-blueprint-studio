...
class RosNodeBase(BaseNode):
    ...
    def _init_template(self, template_key):
        """
        Умная загрузка: переводит старые ключи (cpp_timer_node) 
        в новые универсальные (Timer) и выбирает правильный язык.
        """
        ...
        # 2. НОРМАЛИЗАЦИЯ КЛЮЧА (ПЕРЕВОДЧИК)
        # Если нода просит 'cpp_timer_node', мы превращаем это в 'Timer'
        key_lower = template_key.lower()
        
        final_key = "Default" # Если ничего не совпало
        
        if "timer" in key_lower:
            final_key = "Timer"
        ...
        # 3. ВЫБИРАЕМ СЛОВАРЬ И ЗАБИРАЕМ КОД
        target_dict = CPP_TEMPLATES if is_cpp else PYTHON_TEMPLATES
        
        # Пытаемся найти ключ, если нет — берем Default
